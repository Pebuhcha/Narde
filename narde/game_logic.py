...
class GameBoard:
    ...
    def display_board(self):
        for space in range(1, 25):
            checkers = ''.join(self.board[space])
            print(f"{space}: {checkers}")

    #identifies if the proposed move is valid
    def is_valid_move(self, start, end, startHeadMoves):
        ...
        if self.color == 'w' and (end < 1 or end > 24 or (start == 1 and startHeadMoves <= 0)):
            return False
    ...
    def count_consecutive_checkers(self, start, end):
        ...
        for space in consecutive_area:
            ...
            if end == space or \
                (self.board[space] and self.board[space][0] == self.color and \
                (space != start or len(self.board[space]) >= 2)):
    ...
    def get_possible_moves(self, rolls, startHeadMoves):
        #key=space value=list_of_moves
        possible_moves = dict()
        for start in range(1, 25):
            if self.board[start] and self.board[start][0] == self.color:
                
                #accumulate moves
                possible_moves[start] = []
                for roll in rolls:
                    #get end position after move
                    end = self.calculate_end_position(start, roll)
                    
                    #add if valid move
                    if self.is_valid_move(start, end, startHeadMoves):
    ...
    #black from 24 to 1
    def calculate_end_position(self, start, roll):
        if self.color == 'w':
            return start + roll
        elif self.color == 'b':
            return (start + roll - 1) % 24 + 1
        return None
    ...
    def move_checker(self, start, end):
        ...
        #bears off checker if end >= 25
        if end <= 24:
            self.board[end].insert(0, checker)

    #handles bearing off movement behavior
    def bear_off_moves(self, rolls):
        possible_moves = self.get_possible_moves(rolls, 0)
            ...
            for roll in sorted(rolls, reverse=True):
                ...
                if self.color == 'w':
                    for space in range(25-roll, 25):
                        ...
                        if self.board[space] and self.board[space][0] == 'w':
                    ...
                    for space in range(13-roll, 13):
                        ...
                        if self.board[space] and self.board[space][0] == 'b':
    ...
    def play_turn(self):
        ...
        startHeadMoves = 1
        if self.isFirstTurn and rolls[0] == rolls[1] and rolls[0] in [3, 4, 6]:
            startHeadMoves = 2
        ...
        while rolls:
            ...
            if self.ready_bear_off:
                possible_moves = self.bear_off_moves(rolls)
            else:
                possible_moves = self.get_possible_moves(rolls, startHeadMoves)
            ...
            checker = self.select_checker(possible_moves)
            
            #gather move from user
            move = self.select_move(possible_moves, checker)
            
            #move checker
            self.move_checker(checker, move)
            
            #if checker was moved off the head, count the move
            if (self.color == 'w' and checker == 1) or (self.color == 'b' and checker == 13):
                startHeadMoves -= 1
            
            #update rolls
            if move < 13 and checker > 12:
                #accomodates for the wrapping around of a black checker
                rolls.remove(abs(self.calculate_end_position(checker, move-checker) - checker + 24))
            else:
                rolls.remove(abs(self.calculate_end_position(checker, move-checker) - checker))


    #evaluates if the color is ready to bear
    def check_ready_to_bear(self):
        ...
        finish_tbl_spaces = range(19,25) if self.color == 'w' else range(7,13)

        #sum up the number of checkers on the finishing board
        for space in finish_tbl_spaces:
            if self.board[space] and self.board[space][0] == self.color:
                finish_tbl_count += len(self.board[space])
        
        #if all checkers are present, that color is ready to bear (rtb)
        if finish_tbl_count == 15:
            self.ready_bear_off = True
    ...
    def main():
        ...
        #announce winner
        #print(f"{swinner.upper()} is the winner!")
